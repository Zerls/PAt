...
#include <map>
using namespace std;
static map<string, int> stringToInt;
static map<int, string> intToString;
static map<string, int> ans;
static int idNumber = 1, k;
static int stoifunc(string s) {
    if(stringToInt[s] == 0) {
...
static int G[2010][2010], weight[2010];
static bool vis[2010];
static void dfs(int u, int &head, int &numMember, int &totalweight) {
    vis[u] = true;
    numMember++;
